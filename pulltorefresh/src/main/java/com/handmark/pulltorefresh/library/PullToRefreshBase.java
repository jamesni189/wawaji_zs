...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	//支持自动加载功能 start--------------
	protected boolean mAutoLoadMoreEnable = false;
	
	/**
	 * 该方法调用的地方有俩处
	 * 1、在上拉刷新的时候，设置为true
	 * 2、在下拉的时候，如果没有下一页了，即分页数据已经全部加载完，设置为false
	 * @param enable
	 */
	...
	public void onLastItemVisiable(){
	    ...
	    //如果mode 上拉加载更多已经关闭，不需要进行任何操作
	    if (mMode == Mode.BOTH || mMode == Mode.PULL_FROM_END || mMode == Mode.PULL_UP_TO_REFRESH)
        {
            mCurrentMode = Mode.PULL_FROM_END;
