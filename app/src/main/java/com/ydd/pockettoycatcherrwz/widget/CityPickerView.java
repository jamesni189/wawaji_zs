...
/**
 * 省市区三级选择 作者：liji on 2015/12/17 10:40 邮箱：lijiwork@sina.com
 */
public class CityPickerView implements CanShow, OnWheelChangedListener {
	...
	public CityPickerView(CityConfig config) {
		...
		if (!TextUtils.isEmpty(config.getTitleBackgroundColorStr())) {
			mRelativeTitleBg.setBackgroundColor(
					Color.parseColor(config.getTitleBackgroundColorStr()));
		}
		...
		if (!TextUtils.isEmpty(config.getTitleTextColorStr())) {
			mTvTitle.setTextColor(
					Color.parseColor(config.getTitleTextColorStr()));
		}

		// 设置确认按钮文字颜色
		if (!TextUtils.isEmpty(config.getConfirmTextColorStr())) {
			mTvOK.setTextColor(
					Color.parseColor(config.getConfirmTextColorStr()));
		}

		// 设置取消按钮文字颜色
		if (!TextUtils.isEmpty(config.getCancelTextColorStr())) {
			mTvCancel.setTextColor(
					Color.parseColor(config.getCancelTextColorStr()));
		}
		...
		// 解析初始数据
		if (parseHelper == null
				|| parseHelper.getProvinceBeanArrayList().isEmpty()) {
	...
	private void setUpData() {
		...
		if (!TextUtils.isEmpty(config.getDefaultProvinceName())
				&& parseHelper.getProvinceBeenArray().length > 0) {
			for (int i = 0; i < parseHelper
					.getProvinceBeenArray().length; i++) {
				if (parseHelper.getProvinceBeenArray()[i].getName()
						.contains(config.getDefaultProvinceName())) {
					provinceDefault = i;
					break;
				}
			}
		...
		ArrayWheelAdapter arrayWheelAdapter = new ArrayWheelAdapter<ProvinceBean>(
				config.getContext(), parseHelper.getProvinceBeenArray());
	...
	private void updateCities() {
		...
		// 省份选中的名称
		ProvinceBean mProvinceBean = parseHelper
				.getProvinceBeenArray()[pCurrent];
		...
		CityBean[] cities = parseHelper.getPro_CityMap()
				.get(mProvinceBean.getName());
		if (cities == null) {
			return;
		}
		...
		// 设置最初的默认城市
		int cityDefault = -1;
		if (!TextUtils.isEmpty(config.getDefaultCityName())
				&& cities.length > 0) {
		...
		ArrayWheelAdapter cityWheel = new ArrayWheelAdapter<CityBean>(
				config.getContext(), cities);
		// 设置可见条目数量
	...
	private void updateAreas() {
		...
		if (parseHelper.getPro_CityMap() == null
				|| parseHelper.getCity_DisMap() == null) {
			return;
		}

		CityBean mCityBean = parseHelper.getPro_CityMap()
				.get(parseHelper.getProvinceBean().getName())[pCurrent];
		parseHelper.setCityBean(mCityBean);

		DistrictBean[] areas = parseHelper.getCity_DisMap().get(
				parseHelper.getProvinceBean().getName() + mCityBean.getName());
		...
		if (!TextUtils.isEmpty(config.getDefaultDistrict())
				&& areas.length > 0) {
		...
		ArrayWheelAdapter districtWheel = new ArrayWheelAdapter<DistrictBean>(
				config.getContext(), areas);
		// 设置可见条目数量
	...
	public String[] parseAddress(String address) {
		...
		List<ProvinceBean> list = parseHelper.getProvinceBeanArrayList();
		for (ProvinceBean provinceBean : list) {
			if (!TextUtils.isEmpty(provinceBean.getName())
					&& address.startsWith(provinceBean.getName())) {
				province = provinceBean;
				addressInfo[0] = provinceBean.getName();
				address = address.substring(provinceBean.getName().length());
				break;
			}
		}
		if (TextUtils.isEmpty(addressInfo[0])) {
			return null;
		}
		...
		if (ListUtil.isEmpty(cityList)) {
			...
			addressInfo[1] = "";
			addressInfo[2] = "";
			addressInfo[3] = address;
			return addressInfo;
		}
		CityBean city = null;
		for (CityBean cityBean : cityList) {
			if (!TextUtils.isEmpty(cityBean.getName())
					&& address.startsWith(cityBean.getName())) {
				city = cityBean;
				addressInfo[1] = cityBean.getName();
				address = address.substring(cityBean.getName().length());
				break;
			}
		}
		if (TextUtils.isEmpty(addressInfo[1])) {
			addressInfo[1] = "";
			addressInfo[2] = "";
			addressInfo[3] = address;
			return addressInfo;
		...
		// 地区
		List<DistrictBean> districtList = city.getCityList();
		if (ListUtil.isEmpty(districtList)) {
			return null;
		}
		for (DistrictBean districtBean : districtList) {
			if (!TextUtils.isEmpty(districtBean.getName())
					&& address.startsWith(districtBean.getName())) {
				addressInfo[2] = districtBean.getName();
				address = address.substring(districtBean.getName().length());
				break;
			}
		}
		if (TextUtils.isEmpty(addressInfo[2])) {
			addressInfo[2] = "";
			addressInfo[3] = address;
			return addressInfo;
		}
	...
	public void onChanged(WheelView wheel, int oldValue, int newValue) {
		if (wheel == mViewProvince) {
			...
			if (parseHelper != null && parseHelper.getCity_DisMap() != null) {
				DistrictBean mDistrictBean = parseHelper.getCity_DisMap().get(
						parseHelper.getProvinceBean().getName() + parseHelper
								.getCityBean().getName())[newValue];
