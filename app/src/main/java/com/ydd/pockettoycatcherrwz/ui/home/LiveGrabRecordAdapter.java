...
import com.ydd.pockettoycatcherrwz.entity.LiveRecord;
...
public class LiveGrabRecordAdapter extends BaseAdapter {
	...
	public View getView(int i, View view, ViewGroup viewGroup) {
		...
		if (view == null) {
			...
			view = inflater.inflate(R.layout.item_live_record, null);
			viewHoler.dollimg = (ImageView) view
					.findViewById(R.id.live_fragment_drag_img);
		...
		viewHoler.tvUsername.setText(records.get(i).nickname);
		viewHoler.tvDollStatus
				.setText(records.get(i).status == 0 ? "抓取成功" : "抓取失败");
		viewHoler.tvDollStatus.setTextColor(records.get(i).status == 0
				? context.getResources().getColor(R.color.common_text_red)
				: Color.BLACK);
		if (!TextUtils.isEmpty(records.get(i).createTime)) {
//			String[] times = records.get(i).createTime.split(" ");
//			if (times.length == 1) {
//				viewHoler.tvTime.setText(times[0]);
//			} else {  times[0] + "" + times[1]
				viewHoler.tvTime.setText(records.get(i).createTime);
	//		}
		}
		return view;
	}
	 ...
	 * @param datas
	 */
	public void refreshUi(List<LiveRecord> datas) {
		records.clear();
	 ...
	 * @param datas
	 */
	public void refreshUiByAdd(List<LiveRecord> datas) {
		if (!ListUtil.isEmpty(datas)) {
			records.addAll(datas);
		}
		notifyDataSetChanged();
