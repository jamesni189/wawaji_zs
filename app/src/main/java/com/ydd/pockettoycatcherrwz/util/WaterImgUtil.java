...
public class WaterImgUtil {
	 ...
	 * @return
	 */
	public static Bitmap createWaterMaskRightBottom(Context context, Bitmap src,
			Bitmap watermark, int paddingRight, int paddingBottom) {
		return createWaterMaskBitmap(src, watermark,
	...
	public static Bitmap drawInviteSuccessCode(Context context, Bitmap bitmap,
			String text) {
		Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
		paint.setColor(
				context.getResources().getColor(R.color.common_text_red));
		paint.setTextSize(dp2px(context, 16));
		Rect bounds = new Rect();
		paint.getTextBounds(text, 0, text.length(), bounds);
		return drawTextToBitmap(context, bitmap, text, paint, bounds, 365*bitmap.getWidth()/750f,
				1271*bitmap.getHeight()/1334f+25);
	}

	public static Bitmap drawInviteFailedCode(Context context, Bitmap bitmap,
			String text) {
		Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
		paint.setColor(
				context.getResources().getColor(R.color.common_text_red));
		...
		Rect bounds = new Rect();
		paint.getTextBounds(text, 0, text.length(), bounds);
		LogUtil.printJ("first bitmap width="+bitmap.getWidth()+" height="+bitmap.getHeight());
		return drawTextToBitmap(context, bitmap, text, paint, bounds, 477*bitmap.getWidth()/750f,
				878*bitmap.getHeight()/1334f+25);
	}
	...
	public static Bitmap drawTextToLeftBottom(Context context, Bitmap bitmap,
			String text, int size, int color, int paddingLeft,
			int paddingBottom) {
		Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
		paint.setColor(color);
		paint.setTextSize(dp2px(context, size));
	...
	private static Bitmap drawTextToBitmap(Context context, Bitmap bitmap,
			String text, Paint paint, Rect bounds, float paddingLeft,
			float paddingTop) {
		android.graphics.Bitmap.Config bitmapConfig = bitmap.getConfig();
		LogUtil.printJ("width="+bitmap.getWidth()+" height="+bitmap.getHeight());
